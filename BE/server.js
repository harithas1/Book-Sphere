...
const User = ({ token, role, id }) => {
  ...
  const [allFilters, setAllFilters] = useState(["all"]); // All available filters (book_type)
  ...
  // Fetch available books based on selected filter (book_type)
  useEffect(() => {
    if (!token || !id) return;
    setLoading(true);
    axios
      .get(
        `https://book-sphere-1.onrender.com/user/${id}/books/${selectedFilter}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      )
      .then((response) => {
        ...
        setAllFilters(["all", ...response.data.filters]); // Setting distinct filters from the backend
      })
      .catch((err) => {
        setError(err.message);
      })
      ...
      {activeTab === "rent" && (
        <div className="bg-white p-6 rounded-lg shadow-lg mb-6">
          ...
          {availableBooks.length ? (
            <ul className="space-y-4">
              {availableBooks.map(
                ({ id, title, author, book_type, available_copies }) => (
                  <section key={id} className="relative">
                    ...
                    >
                      ...
                      <p>
                        <strong>Book Type:</strong> {book_type}
                      </p>
                      <p>
